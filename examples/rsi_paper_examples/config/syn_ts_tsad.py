...
def syn_tsad_rsi(data_file, ssm_model_path, train_rsi_model):
    ...
    with open(ssm_model_path, "rb") as f:
        model_dict = pickle.load(f)

    # Determine if it is the simple or complex time series
    if "simple" in data_file:
        print("Detecting anomaly on simple time series data...")
        lstm_lbl = 13
    elif "complex" in data_file:
        print("Detecting anomaly on complex time series data...")
        lstm_lbl = 12

    LSTM = LstmNetwork(
            look_back_len=lstm_lbl,
            num_features=2,
            num_layer=1,
            num_hidden_unit=50,
            device="cpu",
    ...
    if train_rsi_model:
        ...
        hsl_tsad_agent.collect_synthetic_samples(num_time_series=100, save_to_path='data/hsl_tsad_training_samples/itv_learn_samples_syn_ts.csv')
        hsl_tsad_agent.learn_intervention(training_samples_path='data/hsl_tsad_training_samples/itv_learn_samples_syn_ts.csv', 
                                            save_model_path='saved_params/NN_detection_model_syn_ts.pkl', max_training_epoch=20)
    else:
        # Used same hyperparameters as pre-trained model used in the paper
        if "simple" in data_file:
    ...
    # Put red shape after time[time_anomaly]
    ax0.axvspan(time[time_anomaly], time[-1], color='red', alpha=0.2, label='Anomaly')
    ax0.set_xticklabels([])
